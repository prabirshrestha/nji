...
namespace Ionic
{
    ...
    public class Tar
    {
        ...
        public static System.Collections.ObjectModel.ReadOnlyCollection<TarEntry>
            Extract(string archive, string destination = null)
        {
            return ListOrExtract(archive, true, null, destination).AsReadOnly();
        }
                                                    ...
                                                    string destination = null)
        {
            ...
            return t._internal_ListOrExtract(archive, wantExtract, destination);
        }
        ...
        private List<TarEntry> _internal_ListOrExtract(string archive, bool wantExtract, string destination = null)
        {
                        ...
                        switch (entry.Type)
                        {
                            ...
                            case TarEntryType.File_Contiguous:
                                ...
                                if (!String.IsNullOrEmpty(p))
                                {
                                    if (!string.IsNullOrWhiteSpace(destination))
                                    {
                                        p = Path.Combine(destination, p);
                                    }
                                    if (!Directory.Exists(p))
                                        Directory.CreateDirectory(p);
                                }
                                if (output != null) output.Close();
                                output = _internal_GetExtractOutputStream(name, destination);
                                break;
        ...
        private Stream _internal_GetExtractOutputStream(string name, string destination)
        {
            if (!string.IsNullOrWhiteSpace(destination))
                name = Path.Combine(destination, name);
